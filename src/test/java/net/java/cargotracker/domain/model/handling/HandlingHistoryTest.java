...
public class HandlingHistoryTest {
    ...
    Voyage voyage = new Voyage.Builder(new VoyageNumber("X25"),
            SampleLocations.HONGKONG).addMovement(SampleLocations.SHANGHAI,
            new Date(), new Date()).addMovement(SampleLocations.DALLAS,
            new Date(), new Date()).build();
    HandlingEvent event1 = new HandlingEvent(cargo, DateUtil.toDate("2009-03-05"),
            new Date(100), HandlingEvent.Type.LOAD,
    ...
    HandlingEvent event2 = new HandlingEvent(cargo, DateUtil.toDate("2009-03-10"),
            new Date(150), HandlingEvent.Type.UNLOAD,
            SampleLocations.DALLAS, voyage);
    HandlingHistory handlingHistory = new HandlingHistory(asList(event2,
            event1, event1duplicate));

    @Test
    public void testDistinctEventsByCompletionTime() {
        org.junit.Assert.assertEquals(Arrays.asList(event1, event2),
                handlingHistory.getDistinctEventsByCompletionTime());
    }

    @Test
    public void testMostRecentlyCompletedEvent() {
        org.junit.Assert.assertEquals(event2,
                handlingHistory.getMostRecentlyCompletedEvent());
    }
}